...
public:
  ...
  void run(AST *Tree) {
    // The main function has the signature: int main(int, char**)
    ...
    // Create the entry block to attach it to the IRLBuilder
    BasicBlock *BB = BasicBlock::Create(M->getContext(),
                                        "entry", MainFn);
    // Set the insertion point to the entry block
    Builder.SetInsertPoint(BB);

    //Walk the AST and generate the IR
    Tree->accept(*this);
